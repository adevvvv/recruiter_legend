...
const Anketa = () => {
    ...
    const placeholder = 'Пример: Java  Spring Framework  Kubernetes  ORACLE ' +
    ...
    const baseInfo = [
        ...
        },
        {
            ...
            width: '401px',
            marginTop: '200px',
            date: ['Выберите уровень владения',
                '(А1) – начальный',
                '(А2) – ниже среднего',
                '(В1) – средний',
                '(В2) – выше среднего',
    ...
    const [selectInfo, setSelectInfo] = useState([
        {
            ...
            date: ['Выберите гражданство...',
                "Абхазия", "Австралия", "Австрия", "Азербайджан", "Албания", "Алжир", "Ангола", "Андорра", "Аргентина",
                "Армения", "Афганистан", "Багамские Острова", "Бангладеш", "Барбадос", "Бахрейн", "Беларусь", "Белиз",
                "Бельгия", "Болгария", "Боливия", "Босния и Герцеговина", "Бразилия", "Бруней Даруссалам", "Буркина Фасо",
                "Великобритания", "Венгрия", "Венесуэла", "Вьетнам", "Габон", "Гвинея", "Германия", "Государство Палестина",
                "Греция", "Грузия", "Дания", "Демократическая Республика Конго", "Джибути", "Доминиканская Республика", "Другое",
                "Египет", "Замбия", "Израиль", "Индия", "Индонезия", "Иордания", "Ирак", "Иран", "Ирландия", "Исландия",
                "Испания", "Италия", "Йемен", "Казахстан", "Камбоджа", "Канада", "Катар", "Кипр", "Китай", "Колумбия",
                "Кооперативная Республика Гайана", "Королевство Саудовская Аравия", "Кувейт", "Кыргызстан", "Лаос", "Латвия",
                "Ливан", "Ливия", "Литва", "Лихтенштейн", "Люксембург", "Македония", "Малайзия", "Мали", "Мальдивская Республика",
                "Мальта", "Марокко", "Мексика", "Молдова", "Монако", "Монголия", "Мьянма", "Намибия", "Непал", "Нигерия",
                "Нидерланды", "Новая Зеландия", "Норвегия", "ОАЭ", "Объединенная Республика Танзания", "Оман", "Острова Кайман",
                "Пакистан", "Панама", "Парагвай", "Перу", "Польша", "Португалия", "Республика Бенин", "Республика Гаити",
                "Республика Гамбия", "Республика Гана", "Республика Гватемала", "Республика Зимбабве", "Республика Камерун",
                "Республика Кения", "Республика Конго", "Республика Коста-Рика", "Республика Кот-д’Ивуар", "Республика Куба",
                "Республика Либерия", "Республика Маврикий", "Республика Мадагаскар", "Республика Малави", "Республика Нигер",
                "Республика Никарагуа", "Республика Северный Судан", "Республика Сейшельские острова", "Республика Сенегал",
                "Республика Сьерра-Леоне", "Республика Чад", "Республика Эль-Сальвадор", "Реюньон", "Россия", "Румыния",
                "Сан-Томе и Принсипи", "Сент-Винсент и Гренадины", "Сербия", "Сингапур", "Сирия", "Словакия", "Словения", "США",
                "Таджикистан", "Таиланд", "Тайвань", "Тоголезская Республика", "Тунис", "Туркменистан", "Турция", "Уганда",
                "Узбекистан", "Украина", "Уругвай", "Филиппины", "Финляндия", "Франция", "Хорватия", "Центральноафриканская Республика",
                "Черногория", "Чехия", "Чили", "Швейцария", "Швеция", "Шотландия", "Шри-Ланка", "Эквадор", "Эритрея", "Эстония",
                "Эфиопия", "ЮАР", "Южная Корея", "Южная Осетия", "Ямайка", "Япония"
            ],
            width: '296px'
        },
        {
    ...
    const [work, setWork] = useState([<WorkExperience handleChange={handleChange}/>]);

    const addWorkExperience = () => {
        setWork(work => [...work, <WorkExperience handleChange={handleChange}/>]);
    }
            ...
            <form action="" onSubmit={handleSubmit}>
                <div style={{display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center'}}>
                    <div className={styles['container']}>
                        ...
                        <div style={{
                            display: 'grid', gridTemplateColumns: '40% 60%', justifyContent: 'right',
                                ...
                                >
                                    {
                                        baseInfo.slice(3, 5).map((el, i) =>
                                            <BlockInput handleChange={handleChange} type={el.type}
                                                        id={el.name} width={el.width}
                                                        title={el.title}/>)
                                    }
                                    ...
                                    {
                                        selectInfo.map((el, i) =>
                                            <BlockSelect handleChange={handleChange} title={el.title} id={el.name}
                                                         width={el.width}
                                                         date={el.date}/>
                                        )
                                    }
                                ...
                                <div style={{display: 'flex', flexDirection: 'column', textAlign: 'left',}}>
                                    ...
                                    <div style={{display: 'flex', justifyContent: 'space-between', width: '401px'}}>
                                        <button type={'button'}
                                                ...
                                                onClick={() => setDataResume({...dataResume, gender: 'male'})}
                                        >Мужской
                                        </button>
                                        <button type={'button'}
                                                className={`${dataResume.gender === 'female' ? styles['selectedGender'] : null}`}
                                                onClick={() => setDataResume({...dataResume, gender: 'female'})}
                                        >Женский
                                        </button>
                                    </div>
                        ...
                        <div style={{display: 'flex', justifyContent: 'center'}}>
                            <input name={"skills"} onChange={handleChange} onBlur={onBlurSkills} onClick={onClickSkills}
                                   className={styles['inputSkills']} type="text" placeholder={placeholderSkills}/>
                        </div>
                        ...
                        <div style={{display: 'flex', justifyContent: 'center', marginTop: '103px'}}>
                            <button onClick={handleSubmit} className={styles['btnSubmit']} type={'button'}>Сохранить
                            </button>
                        </div>
